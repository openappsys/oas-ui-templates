...
4. **终端块保持深色**（`#0F172A` 底），作为浅色页视觉焦点；终端内 `$` 提示符用 `#7EB6FF`，`✓` 成功行保持 `#4ADE80`（深底上亮绿）；正文/命令文字（`.terminal-body`/`.t-cmd`）固定 `#E2E8F0`、辅助输出（`.t-out`）与终端栏标题（`.terminal-bar`）固定 `#94A3B8`，均不引用 token、深底专用；终端红点/黄点/绿点 macOS 三色不变
