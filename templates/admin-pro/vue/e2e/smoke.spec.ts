// e2e/smoke.spec.ts —— 冒烟用例（7 例）
// 移植自 vanilla-html/e2e/smoke.spec.ts，断言语义逐条对齐。
// 适配记录（子集模版路由表仅 11 条，无 /system/*）：
...
test('viewer 访问受限页显示 403', async ({ page }) => {
  ...
  // 子集模版无 /system/roles（vanilla 原例），viewer 受限页取 /products（roles:['admin']）
