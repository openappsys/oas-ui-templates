// e2e/form.spec.ts —— 创建订单向导/订单详情用例（3 例）
// 移植自 vanilla-html/e2e/form.spec.ts，断言语义逐条对齐。
// 适配记录：vanilla 的 navigate(path) 跳转在本模版为 vue-router 通道（router.push），
// hash URL 形态一致（/#/result、/#/order-detail）；vue 版订单详情链接带 id 查询参数
// （/#/order-detail?id=SO-xxxx），waitForURL 改用正则匹配 hash 路径段。
