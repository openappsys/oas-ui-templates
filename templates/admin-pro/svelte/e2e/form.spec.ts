// e2e/form.spec.ts —— 表单页用例（移植自 react/e2e/form.spec.ts，断言语义逐条对齐）：
// 三步向导 → /result 成功态、校验拦截、订单抽屉跳订单详情页
// Svelte 适配记录：oas-checkbox host 中心合成点击有死区——点击 shadow 内 input
// （与 react 版同款写法逐字沿用）
