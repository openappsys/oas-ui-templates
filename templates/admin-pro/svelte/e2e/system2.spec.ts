// e2e/system2.spec.ts —— 系统管理页用例二（移植自 react/e2e/system2.spec.ts，断言语义逐条对齐）：
// 部门/字典/商品分类 CRUD 与校验、用户详情、操作权限与数据权限
// Svelte 适配记录：svelte 版部门详情卡无 data-md-action 修饰（react 版特有标记），
// 删除按钮改经 popconfirm 内 oas-button 触发（#md-del-pop oas-button，语义等价）；
// 分类删除 popconfirm 沿用 [data-testid][open] [part="ok"] 穿透 shadow 点击，
// 行数断言用 expect.poll（不堆 waitForTimeout）
...
test('admin 部门管理：新建部门入树 + 删除有子部门拦截', async ({ page }) => {
  ...
  // svelte 版无 data-md-action 标记：删除按钮即 popconfirm 的触发按钮
