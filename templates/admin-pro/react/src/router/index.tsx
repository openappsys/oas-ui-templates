// src/router/index.tsx —— 声明式路由组装（HashRouter/BrowserRouter + Routes + 守卫组件）
// 性能决策：v7 data APIs（loader + 异步 transition）每次导航引入 ~300ms 提交延迟
// （纯同步守卫用不上异步 loader，实测 settings 纯渲染页首次就绪 343ms vs vue 45ms），
// 回退声明式模式——同步提交，守卫语义不变。guard() 纯函数与单测保持不动。
...
// （门户 pnpm dev 以 --base /admin-pro/react/ 挂子路径时实测踩中），勿回退
...
/** 已登录兜底：未知路径 → /not-found。例外：URL 停在 /login 的登录瞬移窗口
 *  （session 更新后、login.tsx 手动 navigate 前）保持原「匹配落空渲染 null」行为，
 *  否则兜底抢跑致登录后落错页 */
      ...
      <Routes>
        <Route element={<AppShell />}>
          ...
          {/* 兜底：未知路径 → /not-found（对齐 vue 版 /:pathMatch(.*)* 重定向）。
              已登录下 /login 不在路由表（登录页只存在于未登录分支），
              登录瞬移窗口由 login.tsx 手动 navigate 接管 */}
