// e2e/tabs.spec.ts —— 多页签用例（移植自 vanilla-html/e2e/tabs.spec.ts，断言语义逐条对齐）
// 选择器适配：本模版路由表无订单页（vanilla 的 /orders、/order-detail），
// 导航目标改用 /products + /users；「隐藏路由归父」用例改用本模版唯一的
// hidden+parent 路由 /products/edit（parent=/products，form-mode=page 下新建商品进入）
