// e2e/smoke.spec.ts —— 冒烟用例（移植自 vanilla-html/e2e/smoke.spec.ts，断言语义逐条对齐）
// 选择器适配：viewer 403 目标路由由 vanilla 的 /system/roles 改为本模版唯一的 admin-only
// 路由 /products（本模版路由表无 system/*，见 src/router/routes.tsx）
