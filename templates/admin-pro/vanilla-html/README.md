...
功能：登录（角色：管理员 / 只读访客）· hash 路由权限守卫 · 仪表盘（统计卡 + ECharts + 最近订单）· 用户管理（搜索 / 分页 / 弹窗 CRUD）· 个人中心 · light/dark 主题 · 移动端抽屉侧栏。
...
- `import '@oas-ui/ui'` 为全量注册；追求体积可改为按组件子路径引入（见 oas-ui 文档「按需引入」）
