...
test('商品管理：卡片/列表双视图切换，列表含开关与操作列', async ({ page }) => {
  ...
  expect(await page.evaluate(() => localStorage.getItem('oas-admin-cdn.products-view'))).toBe(
    'table',
  )
...
test('商品管理：dialog 模式下新建表单以对话框呈现', async ({ page }) => {
  const errors = await noConsoleErrors(page)
  await login(page)
...
test('商品管理：page 模式下新建跳转整页表单', async ({ page }) => {
  const errors = await noConsoleErrors(page)
  await login(page)
...
test('商品管理：批量上架/下架翻转选中项', async ({ page }) => {
  const errors = await noConsoleErrors(page)
  await login(page)
  await openProductsTable(page)
  const table = page.getByTestId('product-table')
...
test('商品管理：列设置显隐列并持久化', async ({ page }) => {
  const errors = await noConsoleErrors(page)
  await login(page)
  await openProductsTable(page)
  const table = page.getByTestId('product-table')
  ...
  expect(await page.evaluate(() => localStorage.getItem('oas-admin-cdn.products.columns'))).toBe(
