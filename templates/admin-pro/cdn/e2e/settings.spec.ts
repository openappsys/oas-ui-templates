...
// 适配点：cdn 壳层暂无多页签栏（无 .tabs-bar/#page-tabs）——「关闭多页签栏」用例仅断言
...
test('设置中心：表单呈现/密度/字号切换写入 localStorage', async ({ page }) => {
  ...
  expect(await page.evaluate(() => localStorage.getItem('oas-admin-cdn.form-mode'))).toBe('dialog')
  // 密度与字号在「外观」tab
  ...
  expect(
    await page.evaluate(() => localStorage.getItem('oas-admin-cdn.settings.table-density')),
  ).toBe('compact')
  await page.getByTestId('font-size-group').getByText('特大').click()
  expect(await page.evaluate(() => localStorage.getItem('oas-admin-cdn.settings.font-size'))).toBe(
    'xl',
  )
...
test('设置中心：多页签栏开关写入 localStorage（cdn 壳层暂不消费）', async ({ page }) => {
  ...
  expect(await page.evaluate(() => localStorage.getItem('oas-admin-cdn.settings.tabs-bar'))).toBe(
    'false',
  )
  await page.getByTestId('tabs-bar-toggle').click()
  expect(await page.evaluate(() => localStorage.getItem('oas-admin-cdn.settings.tabs-bar'))).toBe(
...
test('设置中心：主题色按明暗主题独立存储并联动生效', async ({ page }) => {
  ...
  expect(
    await page.evaluate(() => localStorage.getItem('oas-admin-cdn.settings.theme.light')),
  ).toBe('#7c3aed')
  ...
  expect(
    await page.evaluate(() => localStorage.getItem('oas-admin-cdn.settings.theme.dark')),
  ).toBeNull()
  expect(errors).toEqual([])
})
