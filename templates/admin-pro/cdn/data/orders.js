/**
 * 订单数据模块（自 vanilla src/data/orders.ts 去 TS 移植，逻辑与种子数据逐字保留）
 * localStorage 键名前缀按 cdn 既有约定改为 oas-admin-cdn.*
 */
import { persist, restore } from './store.js'

/** @typedef {'pending' | 'paid' | 'shipping' | 'done' | 'cancelled'} OrderStatus */
/**
 * @typedef {Object} OrderRow
 * @property {string} id
 * @property {string} customer
 * @property {number} amount
 * @property {OrderStatus} status
 * @property {string[]} items
 * @property {string} created
 * @property {string} creator
 * @property {boolean} [urgent]
 * @property {string} [phone]
 * @property {string} [note]
 */

/** @param {Date} d */
function iso(d) {
  return d.toISOString().slice(0, 10)
}

function today() {
  return iso(new Date())
}

/** @param {number} n @param {number} [day] */
function monthOffset(n, day = 15) {
  const d = new Date()
  d.setMonth(d.getMonth() - n)
  d.setDate(day)
  return iso(d)
}

function seed() {
  const raw = [
    ['华信科技', 12800, 'done', ['数码相机', '无线充电器'], today()],
    ['蓝海贸易', 8600, 'shipping', ['蓝牙耳机', '便携音箱'], today()],
    ['星野文化', 3200, 'pending', ['机械键盘'], today()],
    ['晨光实业', 21500, 'done', ['办公桌', '人体工学椅', '台灯'], monthOffset(1)],
    ['云图软件', 6900, 'paid', ['显示器', '扩展坞'], monthOffset(1)],
    ['远帆教育', 4500, 'pending', ['平板电脑'], monthOffset(1)],
    ['基石建筑', 9800, 'cancelled', ['安全帽', '防护手套'], monthOffset(1)],
    ['微澜传媒', 15600, 'paid', ['相机云台', '补光灯'], monthOffset(2)],
    ['青禾餐饮', 3200, 'shipping', ['咖啡机'], monthOffset(2)],
    ['启睿咨询', 7600, 'cancelled', ['投影仪'], monthOffset(2)],
    ['拓界物流', 18200, 'paid', ['液压叉车'], monthOffset(3)],
    ['静水深流', 1100, 'pending', ['保温杯', '雨伞'], monthOffset(3)],
  ]
  return raw.map(([customer, amount, status, items, created], i) => ({
    id: `SO-${String(10001 + i)}`,
    customer,
    amount,
    status,
    items,
    created,
    creator: i % 2 === 0 ? '张伟' : '王芳',
    urgent: i % 3 === 0,
    phone: `138${String(10000000 + i * 123456).slice(-8)}`,
    note: i % 4 === 0 ? '请在工作日配送' : undefined,
  }))
}

const KEY = 'oas-admin-cdn.orders.v1'
const rows = /** @type {OrderRow[]} */ (restore(KEY, seed))
let seq = 10001 + rows.length - 1

/** @param {*} value @param {number} [ms] */
function delay(value, ms = 100) {
  return new Promise((resolve) => setTimeout(() => resolve(value), ms))
}

/** @returns {Promise<OrderRow[]>} */
export function listOrders() {
  return delay([...rows])
}

/** @param {string} id @returns {Promise<OrderRow | null>} */
export function getOrder(id) {
  const row = rows.find((r) => r.id === id)
  return delay(row ? { ...row } : null)
}

/**
 * @param {Omit<OrderRow, 'id' | 'created' | 'creator'> & { creator?: string }} data
 * @returns {Promise<OrderRow>}
 */
export function createOrder(data) {
  const row = {
    ...data,
    id: `SO-${String(++seq)}`,
    created: today(),
    creator: data.creator ?? '张伟',
  }
  rows.unshift(row)
  persist(KEY, rows)
  return delay(row)
}

/** @param {string} id @param {OrderStatus} status @returns {Promise<OrderRow | null>} */
export function updateOrderStatus(id, status) {
  const i = rows.findIndex((r) => r.id === id)
  if (i === -1) return delay(null)
  rows[i] = { ...rows[i], status }
  persist(KEY, rows)
  return delay(rows[i])
}

export function resetOrders() {
  rows.length = 0
  rows.push(...seed())
  seq = 10001 + rows.length - 1
  localStorage.removeItem(KEY)
}
